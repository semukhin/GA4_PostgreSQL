...
def fetch_ad_campaigns_data():
    ...
    metrics = [
        ...
        "returnOnAdSpend",
        "totalPurchasers"  # Будем использовать для расчета конверсий
    ]
    ...
    while current_date <= end_date:
        ...
        for row in rows:
            ...
            ad_group = row.dimension_values[4].value
            ad_group_id = row.dimension_values[5].value
            keyword = row.dimension_values[6].value
            creative_id = row.dimension_values[7].value
            device_category = row.dimension_values[8].value
            ...
            roas = float(row.metric_values[4].value or 0.0)
            conversions = int(row.metric_values[5].value or 0)
            ...
            # Оценка бюджета и CAC
            # Бюджет обычно определяется на уровне кампании, а не дня
            # Мы будем предполагать, что дневной бюджет равен фактическим затратам
            daily_budget = cost
